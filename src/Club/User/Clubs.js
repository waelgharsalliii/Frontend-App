...
export default function Clubs() {
  ...
  const PaymentHandler = async (e, club) => {
    ...
    const data = await response.json();
    const paymentId=data.result.link.substring(data.result.link.lastIndexOf("/") + 1);
          ...
          <div className="div1">
            <Card style={{ width: "18rem", marginRight: "10px" }}>
              ...
              <Card.Body>
                ...
                <button className="btn btn-danger" onClick={(e)=>WindowHandler(e,club)}>
                <span className="icon text-white-50">
                <i class="fas fa-flag"></i>
                  </span>
                  <span className="text" style={{paddingLeft:"5px"}}>Close Payment Window</span></button>
              </Card.Body>
            </Card>
          </div>
        ))
